%
%\VignetteIndexEntry{Rsocialdata Overview}
%\VignetteDepends{}
%\VignetteKeywords{Documentation}
%\VignettePackage{Rsocialdata}
...
The Rsocialdata package provides a class, called Rsocialdata, and some
...
to a Rsocialdata object is provided.
...
\begin{itemize}
  \item MeasureVariable objects should be used instead of numeric vector objects
	...
	\item Rsocialdata objects should be used instead of data.frame objects
\end{itemize}
...
In this section we will present how to handle each variable type. At this end of this section you will be able to create, modify, recode the different kinds of variable, and deal with the Rsocialdata data base format.
...
% \item lorsque l'objet est crée, on rempli les codes avec un outil de vue de data.frame, peut-être en formulaire web, qui remplit le jeu de données, et fait des tests pour dire si l'object est Rsocialdata est prêt à être généré (si il y a des NA, entrées incompletes, ... on ne pourra pas générer l'objet)
% \end{itemize}
...
\section{Rsocialdata Design}

Rsocialdata relies on the R package system to distribute code and
