...
setClass(
  ...
  validity = function(object) {
    if(Rsocialdata.globalenv$print.io) cat (" =>       BinaryVariable: object validity check \n")
  	flag = TRUE
...
binaryVariable <- function(
  ...
  if(Rsocialdata.globalenv$print.io) cat(" => (in)  BinaryVariable: builder \n")
...
#   a1 <- as.missing('versicolor', a)
    ...
    if(Rsocialdata.globalenv$print.io) cat(" => (out) BinaryVariable: builder \n")
    return(out)
}
