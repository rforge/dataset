...
setClass(
  Class = "Variable",
    ...
    if(Rsocialdata.globalenv$print.io) cat (" =>       Variable: object validity check \n")
		flag = TRUE
...
variable <- function(x, values, missings, description) {
  if(Rsocialdata.globalenv$print.io) cat(" => (in)  Variable: virtual builder \n")
...
#   print(values)
  ...
  out <- list(
    ...
    Variable.version = Rsocialdata.globalenv$Variable.version
  )
  #print(out)
  if(Rsocialdata.globalenv$print.io) cat(" => (out) Variable: virtual builder \n")
  return(out)
}
...
setMethod(
  ...
  signature = c("Variable"), 
  definition = function (x, ...) {
    message("A Variable have to be plot inside a Rsocialdata object.")
...
#     if(inherits(args[[1]], "Variable")) {
    ...
    message("")
    message("table() has been desactivated by the Rsocialdata package.")
    ...
    message("")
    stop("table() has been desactivated by the Rsocialdata package.")
