...
setClass(
  ...
  validity = function(object) {
    if(Rsocialdata.globalenv$print.io) cat (" =>       QuantitativeVariable: object validity check \n")
  	flag = TRUE
...
quantitativeVariable <- function(x, values, missings, description) {
  if(Rsocialdata.globalenv$print.io) cat(" => (in)  QuantitativeVariable: virtual builder \n")
  ...
  #print(out)
  if(Rsocialdata.globalenv$print.io) cat(" => (out) QuantitativeVariable: virtual builder \n")
  return(out)
}
...
setMethod("Arith", signature(e1="QuantitativeVariable", e2="QuantitativeVariable"),
  ...
  # then we have to test that we did't create collisions with missings
    ...
    if(nmissings.before != nmissings.after) {
      message(Rsocialdata.globalenv$message.missing.collision)
      message(paste('nmissings.before', nmissings.before))
      message(paste('nmissings.after', nmissings.after))
