...
cunion <- function (..., deparse.level = 1) {
  ...
  if(!all(unlist(mapply(inherits, y, 'Rsocialdata'))))
    stop("[Rsocialdata::cunion] all object must inherit of class 'Rsocialdata'")
...
setMethod(
  f = "merge",
  signature = c("Rsocialdata", "Rsocialdata"), 
...
#   	  stop("The merge method for Rsocialdata objects expect names in x and y are unique. \n Please renames concerned variables.")
    ...
    if(na.in.tuplesX || na.in.tuplesY) {
      new.miss.code <- Rsocialdata:::.missing.gen.candidate.vector(
        valids = unique(c(
          unlist(allvalids(x)),
          unlist(allvalids(y))
        )),
...
#         print(newcodes)
    ...
    return(new(
      ...
      Rsocialdata.version = Rsocialdata:::Rsocialdata.globalenv$Rsocialdata.version
    ))
...
#merge <- function (x, y, ...) {
	# si deux objets Rsocialdata on lance le merge Rsocialdata, sinon on lance le merge normal
#}
