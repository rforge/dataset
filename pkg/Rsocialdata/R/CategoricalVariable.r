...
setClass(
  ...
  validity = function(object) {
    if(Rsocialdata.globalenv$print.io) cat (" =>       CategoricalVariable: object validity check \n")
  	flag = TRUE
...
categoricalVariable <- function(x, values, missings, description) {
  if(Rsocialdata.globalenv$print.io) cat(" => (in)  CategoricalVariable: virtual builder \n")
...
#   names(missings) <- make.names(names(missings), unique=T)
  ...
  #print(out)
  if(Rsocialdata.globalenv$print.io) cat(" => (out) CategoricalVariable: virtual builder \n")
  return(out)
}
...
#       print(newvalids)
    ...
    if(is.null(quiet) || (quiet == FALSE)) {
      message(Rsocialdata.globalenv$message.operation.success)
    }
    message(Rsocialdata.globalenv$message.allocation.rows)
...
setMethod(
  ...
  definition = function (x, data, sort, sort.ordinal, ...) {
    ...
    if(! 'weights' %in% names(l)) {
      ...
      message('Prefer use the method for Rsocialdata objects to take weights into account.')
