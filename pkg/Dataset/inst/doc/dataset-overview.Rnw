...
\title{What is Dataset}
\maketitle

\section*{Prequisites}

The Dataset package provides a class, called Dataset, and some
methods allowing to handle more information about a data set in
...
\begin{itemize}
  \item MeasureFeature objects should be used instead of numeric vector objects
	\item CategoricalFeature objects should be used instead of factor objects
	\item OrdinalFeature objects should be used instead of ordered factor objects
