...
setMethod("summaryToPDF", "Dataset", 
  definition = function (object, pdfSavingName, graphics = FALSE, description.chlength, values.chlength, dateformat, latexPackages, keepTex = FALSE) {
  require(xtable)
  nTuples <- nrow(object)
  ...
  outFileCon <- file(latexFile, "w", encoding="UTF-8")
	...
	latex.head(outFileCon)
  
  if (!missing(latexPackages))
